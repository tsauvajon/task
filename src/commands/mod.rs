...
pub fn run(cli: Cli) -> Result<(), String> {
    ...
    match cli.command {
        Commands::Bootstrap => bootstrap::run(&layout),
        Commands::Doctor => doctor::run(&layout),
        Commands::Clone { repo_url, repo_key } => clone::run(&layout, &repo_url, repo_key),
        Commands::Start {
            repo,
            branch,
            base_ref,
        } => start::run(&layout, &repo, &branch, base_ref.as_deref()),
        Commands::Open { repo, branch } => open::run(&layout, &repo, &branch),
        Commands::Park => park::run(&layout),
        Commands::Path { repo, branch } => path::run(&layout, &repo, &branch),
        Commands::List { repo } => list::run(&layout, repo.as_deref()),
        Commands::Worktrees { repo } => worktrees::run(&layout, repo.as_deref()),
        Commands::Clean {
            repo,
            branch,
            force,
        } => clean::run(&layout, &repo, &branch, force),
        Commands::Prune { repo } => prune::run(&layout, &repo),
        Commands::Done { worktree_path } => done::run(worktree_path.as_deref()),
