...
mod tests {
    ...
    #[test]
    fn run_returns_ok_when_no_repos_exist() {
        ...
        let result = super::run(&env, None);
        assert!(result.is_ok(), "should succeed with no repos: {result:?}");
    }

    #[test]
    fn run_returns_ok_for_bare_repo_with_no_worktrees() {
    ...
    #[test]
    fn run_errors_when_specific_repo_arg_does_not_exist() {
        ...
        let env = env_for(&repos_dir, &wt_dir);
        let result = super::run(&env, Some("github.com/me/nonexistent"));
    ...
    #[test]
    fn run_returns_ok_for_explicit_repo_arg_that_exists() {
        ...
        let env = env_for(&repos_dir, &wt_dir);
        let result = super::run(&env, Some("github.com/me/app"));
        assert!(
            result.is_ok(),
